...
- `client_id` — stable app identifier used for upserts (`onConflict: user_id,client_id`). Remote unique indexes are **non-partial** so PostgREST can infer the conflict target (partial indexes are not supported by the Supabase JS client).
