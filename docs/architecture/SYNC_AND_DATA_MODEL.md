...
The app generates device-owned identifiers (for example `plant-1780497725349-9263nov7`) for SQLite primary keys.

Supabase stores those values in `client_id`, not in `id`:

- `id` — UUID generated by Postgres (`gen_random_uuid()`)
- `client_id` — stable app identifier. The app syncs by looking up `(user_id, client_id)` then **inserting** or **updating by remote UUID** (does not rely on PostgREST `onConflict`, which requires a deployed unique constraint). Migration `20260604120000_client_id_unique_constraints.sql` adds those constraints for DB integrity.

SQLite also keeps `remote_id` on synced entities so foreign keys can be translated to cloud UUIDs during upload.
...
- `services/database/syncEntityPriority.ts`
- `services/database/clientIdMapping.ts`
- `docs/architecture/CLIENT_ID_SYNC_POST_IMPLEMENTATION_AUDIT.md`
